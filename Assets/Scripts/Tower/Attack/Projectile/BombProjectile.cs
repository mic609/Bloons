...
public class BombProjectile : Projectile
{
    ...
    private void OnDisable()
    {
        ...
        var ceiling = 0f;
        if (_numberOfBloonsToPop < bloonsToDestroy.Count)
            ceiling = _numberOfBloonsToPop;
        else
            ceiling = bloonsToDestroy.Count;
        ...
        for (int i = 0; i < ceiling; i++)
        {
            ...
            {
                //Destroy(bloonToDestroy);
                if (bloonToDestroy.GetComponent<BloonController>().GetPopThrough() <= -1)
                {
